...
const DrugForm = ({ onAdd, defaultDrugs = [] }) => {
  ...
  const renderSearchBar = () => (
    <div className="relative">
      <Search className={`absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 ${isMobile ? 'w-5 h-5' : 'w-4 h-4'}`} />
      <input
        type="text"
        value={searchQuery}
        ...
        className={`w-full border rounded-lg focus:ring-2 focus:ring-blue-500 ${
          isMobile ? 'pl-12 pr-12 py-3 text-lg' : 'pl-10 pr-4 py-2'
        }`}
      />
      {searchQuery && (
        <button 
        ...
        >
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <h3 className={`font-medium text-gray-900 ${isMobile ? 'text-lg' : ''}`}>
                {drug.name}
              </h3>
            ...
            {!isMobile && (
              <div className="bg-gray-50 px-3 py-2 rounded">
                ...
                {drug.instructions && drug.instructions.split('\n').map((line, index) => (
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
        <div className={`bg-white ${
          ...
          <div className="p-4 space-y-6">
            {/* Form fields are the same for both mobile and desktop */}
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className={`flex-1 border rounded-lg focus:ring-2 focus:ring-blue-500 ${
                    isMobile ? 'px-4 py-3 text-lg' : 'px-3 py-2'
                  }`}
                  placeholder="Amount"
                  min="0"
                  step="any"
          ...
          <div className="sticky bottom-0 bg-white border-t p-4 flex gap-3">
            <button
              ...
              className={`flex-1 bg-blue-500 text-white rounded-lg hover:bg-blue-600 ${
                isMobile ? 'px-4 py-3 text-lg font-medium' : 'px-4 py-2'
              }`}
            >
              Add Substance
            </button>
